...
class LLMRouter:
    def __init__(self, model_name: str = "llama3.2:latest", circular_context=None):
        self.model_name = model_name
        self.circular_context = circular_context
    ...
    def _extract_json(self, raw: str) -> dict:
        """
        Robust JSON extraction: handles prose before/after the JSON object,
        backtick fences, and minor formatting quirks from chatty LLMs.
        """
        text = raw.strip()
        ...
        # 2. Try direct parse (fast path — works if output is clean JSON)
        try:
            return json.loads(text)
        except json.JSONDecodeError:
            pass

        # 3. Extract the first {...} block — handles preamble/postamble prose
    ...
    def route(
        ...
        """
        # Format circular_context if present
        context_str = ""
        if self.circular_context:
            if isinstance(self.circular_context, list):
                context_str = ", ".join(self.circular_context)
            else:
                context_str = str(self.circular_context)

        system_prompt = (
            "You are a regulatory compliance routing expert for an Indian commercial bank. "
            f"{('This obligation is from: ' + context_str + '. ') if context_str else ''}"
...
"""
        ...
        try:
            ...
            print(f"[LLM ERROR] route() {type(e).__name__}: {e}")
            return {
                "status": "error",
...
"""
        try:
            ...
            print(f"[LLM ERROR] route_correct() {type(e).__name__}: {e}")
            return {
                "status": "error",
                "department": "Unassigned",
                "sub_vertical": None,
